...
def cmd(command: str):
    if command.startswith("print "):
        ...
        dataname = command[5:]
        data(dataname)
    elif command.startswith("listdata"):
        print("cookie.gif")
        ...
        print("print {text} | Prints {text} to the screen")
        print("exit | Exits Quilt")
        print("f {file} | Runs a file (must be Quilt commands)")
        print("os {command} | Runs a command from your OS (eg. taskmgr.exe)")
        print("cookieclicker | Opens Cookie Clicker")
        print("data {data} | Download data")
        print("listdata | List all downloadable data")
        print("domain {domain} | Check if a domain exists")
        print("? | This menu")
    else:
        invalidcmd()
def filecheck(command: str):
    if command.startswith("f "):
        ...
        cmd(command)
key("f11")
color("Quilt 1.4", C.blue)
color("Type ? to see commands", C.blue)
while True:
